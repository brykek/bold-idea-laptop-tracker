    ...
    mysqlPool.getConnection(
      function(err, connection) {
        try {
          ...
          console.log(' Error getting mysqlPool connection in localStrategy: ' + err);
          throw err;
        } finally {
          console.log(' mysqlPool.release() in localStrategy');
          connection.release();
        }
      }
    );
...
const jwtStrategy = new JwtStrategy(opts, (token, done) => {
  try {
    ...
    mysqlPool.getConnection(
      function(err, connection) {
        try {
          connection.query("SELECT * FROM users WHERE id = ?", [token.id], function(err, user) {
            if (err) { return done(err); }
            if (!user) { return done(null, false); }
            return done(null, user);
          });
        } catch (error){
            console.log (' Error getting mysqlPool connection in jwtStrategy: ' + err);
            throw err;
        } finally {
          console.log(' mysqlPool.release() in jwtStrategy');   
          connection.release();
        }
      });
  } catch (e) {
      ...
      mysqlPool.getConnection(
        ...
        function(err, connection){
          try {
            ...
            console.log (' Error getting mysqlPool connection in insert user: ' + err);
            throw err;
          } finally {
            console.log(' mysqlPool.release() in insert user');   
            connection.release();
          }
        }
  ...
  mysqlPool.getConnection(
    function(err, connection){
      try {
        ...
        console.log (' Error getting mysqlPool connection in get all users: ' + err);
        throw err;
      
      } finally {
        console.log(' mysqlPool.release() in get all users');   
        connection.release();
      }
    }
  );
    ...
    { 
      ...
      crypto.pbkdf2(password, salt, 310000, 32, "sha256", function(err, hashedPassword) {
        ...
        mysqlPool.getConnection(
          function (err, connection){
            try {
              connection.query(query, [hashedPassword, salt, id], (err, result) => {
                if (err) next(err);
                res.send();
              });
            } catch (error) {
              console.log (' Error getting mysqlPool connection in update password: ' + err);
              throw err;
            } finally {
              console.log(' mysqlPool.release() in update password');   
              connection.release();
            }
          }
        );
      ...
      mysqlPool.connection(
        function (err, connection){
          try {
            ...
            console.log (' Error getting mysqlPool connection in update role: ' + err);
            throw err;
          } finally {
            console.log(' mysqlPool.release() in update role');   
            connection.release();
          }
        }
      );
  ...
  mysql.getConnection(
    ...
    function(err, connection){
      try {
        ...
        console.log (' Error getting mysqlPool connection in delete user: ' + err);
        throw err;
      } finally {
        console.log(' mysqlPool.release() in delete user');   
        connection.release();
      }
    }
  );
  ...
  mysqlPool.getConnection(
    function(err, connection){
      try {
        ...
        console.log (' Error getting mysqlPool connection in inventory: ' + err);
        throw err;
      } finally {
        console.log(' mysqlPool.release() in inventory');   
        connection.release();
      }
    }
  ...
  mysqlPool.getConnection(
    function(err, connection) {
      try {
        ...
        console.log (' Error getting mysqlPool connection in inventory id: ' + err);
        throw err;
      } finally {
        console.log(' mysqlPool.release() in inventory id');   
        connection.release();
      }
    }
  );
...
app.post("/add", authJwt, (req, res, next) => {
  ...
  mysqlPool.getConnection(
    ...
    function(err, connection){
      try {
        connection.query(sqlQuery, laptop, (err, results) => {
          if (err) {next(err)}
          else return res.status(201).send();
        });
      } catch (error) {
        console.log (' Error getting mysqlPool connection in add laptop: ' + err);
        throw err;
      } finally {
        console.log(' mysqlPool.release() in add laptop');   
        connection.release();
      }
    }
  );
...
app.put("/edit/:id", authJwt, (req, res, next) => {
  ...
  mysqlPool.getConnection(
    function(err, connection) {
      try {
        connection.query(sqlQuery,[laptop, id], (err, results) => {
          if (err) {
            console.log(err);
            next(err);
          } else {
        ...
        console.log (' Error getting mysqlPool connection in update by id: ' + err);
        throw err;
      } finally {
        console.log(' mysqlPool.release() in update by id');   
        connection.release();
      }
    }
  ...
  mysqlPool.getConnection(
    function(err, connection){
      try {
        ...
        console.log (' Error getting mysqlPool connection in dropdown: ' + err);
        throw err;
      } finally {
        console.log(' mysqlPool.release() in dropdown');   
        connection.release();
      }
    }
  );
  ...
  mysqlPool.getConnection(
    ...
    function(err, connection){
      try {
        ...
        connection.query(sqlQuery, [option], (err, results) => {
          if (err) next(err)
  ...
  mysqlPool.getConnection(
    ...
    function(err, connection){
      try {
        ...
        console.log (' Error getting mysqlPool connection in delete: ' + err);
        throw err;
      } finally {
        console.log(' mysqlPool.release() in delete');   
        connection.release();
      }
    }
  );
